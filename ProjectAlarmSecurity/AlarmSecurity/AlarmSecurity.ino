...
void setup(){
    //----------------------- Servo (แสดงค่า)
    myservo.attach(4,0,180);                            // กำหนดขาสัญญาณ(Pin4)และกำหนดความกว้างในการหมุนที่ 0 องศาและ 180 องศา (pin,min,max)
    //----------------------- SevenSegment (แสดงค่า)
    sevenSegment.init();                                // จัดเก็บรับค่าใน()
    sevenSegment.set(7);                                // ระดับความสว่าง0-7; 
    //----------------------- Ultrasonic (แสดงค่า)  
...
void loop(){
    ...
    for(degree = 0; degree < 180; degree += 1)          
    {
        ...
        duration = pulseIn(echoPin , 1);                //รับค่าหรืออ่าน echoPin เพื่อส่งคืนเวลาเดินทางของคลื่นเเหนือสียงเป็นไมโครวินาที
...
// ขั้นตอน Ultrasonic ใช้ขา Digital 2 ขา ขาแรกจะทำการ Trig สัญญาณเป็นลอจิคออกไปเป็นระยะเวลาสั้นมากๆแล้วทำการ จับเวลาคลื่นที่สะท้อนกลับมาที่ ขา Echo แล้วนำเวลาที่ได้มาคำนวนหาระยะทางจากสมการ
